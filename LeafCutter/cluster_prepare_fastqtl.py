...
def gtf_to_bed(annotation_gtf):
    
    chrom = []
    start = []
    end = []
    gene_id = []
    with gzip.open(annotation_gtf, 'rt') as gtf: #added gzip. to open .gz file and added 't' to handle as txt
        for row in gtf:
            row = row.strip().split('\t')
            #if row[0][0] =='#' or row[2]!=feature: continue #Skip header and non gene lines
            chrom.append(row[0])

            # TSS: gene start (0-based coordinates for BED)
            if row[3]=='+': #[6]
                start.append(np.int64(row[1])-1) #[3]
                end.append(np.int64(row[1])) 
            elif row[3]=='-': #[6]
                start.append(np.int64(row[2])-1)  # last base of gene [4]
...
def write_bed(bed_df, output_name):
    ...
    bgzip = subprocess.Popen('bgzip -c > '+output_name,
        stdin=subprocess.PIPE, shell=True, universal_newlines=True) #Added universal_newlines=True
    #bed_df.to_csv(bgzip.stdin, sep='\t', index=False)
    stdout, stderr = bgzip.communicate(bed_df.to_csv(bgzip.stdin, sep='\t', index=False)) #Added parameter
...
if __name__=='__main__':
    ...
    parser.add_argument('--leafcutter_dir', default='/project/cphg-millerlab/CAD_QTL/coronary_QTL/transcriptome/LeafCutter/LeafCutter_scripts',
    ...
    with cd(args.output_dir):
        ...
        with tempfile.NamedTemporaryFile(dir=args.output_dir) as tmp:
            ...
            subprocess.check_call(
                'python '+os.path.join(args.leafcutter_dir, 'clustering', 'leafcutter_cluster_regtools.py' \
                    +' --juncfiles '+tmp.name \
        ...
        pd.Series(group_s).sort_values().to_csv(os.path.join(args.output_dir, args.prefix+'.leafcutter.phenotype_groups.txt'), sep='\t', header=False)
